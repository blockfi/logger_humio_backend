...
* **debug_io_device**: `pid()`, `:stdio`, `:stderr`, or `nil`. The IO device to which error messages are sent if sending logs to Humio fails for any reason. If set to `nil`, no debug information will be logged. [default: `nil`]
